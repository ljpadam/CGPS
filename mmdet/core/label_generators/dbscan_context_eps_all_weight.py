...
@torch.no_grad()
def label_generator_dbscan_context_eps_all_weight(cfg, features, cuda=True, indep_thres=None, all_inds=None, epoch=0, eps=None, **kwargs):
    assert cfg.PSEUDO_LABELS.cluster == "dbscan_context_eps_all_weight"
    ...
    # clustering
    lmd = cfg.PSEUDO_LABELS.lmd
    if eps is not None:
        print("pre eps", eps)
        cfg_eps = cfg.PSEUDO_LABELS.eps
        eps = lmd * cfg_eps[1] + (1 - lmd) * eps
        print("after eps", eps)
