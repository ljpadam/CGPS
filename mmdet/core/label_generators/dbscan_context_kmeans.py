...
def process_label_with_context(labels, centers, features, inds, num_classes):
    # if the persons in the same image are clustered in the same clust, use kmenas to split them
    ...
    for uid in unique_inds:
        ...
        if len(dups) > 0:
            ...
            init_features = features[tmp_id[dup[1]].squeeze(1)].cpu().numpy()
            feat_id = labels == dup[0]
            feat_id = feat_id.nonzero()
            tmp_features = features[feat_id].squeeze(1).cpu().numpy()
            #print(tmp_features.shape)
            #tmp_features = tmp_features.squeeze(1)
            #print(tmp_features.shape)
            km = KMeans(n_clusters=len(dup[1]),init=init_features, n_init=1).fit(tmp_features)
            ulabels = sorted(list(set(list(km.labels_))))
            #print(km.labels_, ulabels, km.cluster_centers_.shape)
            for ulb in ulabels:
                label_id = km.labels_ == ulb
                tmp_label_id = label_id.nonzero()
                if ulb > 0:
                    labels[feat_id[tmp_label_id]] = num_classes
                    centers = torch.cat((centers, torch.from_numpy(km.cluster_centers_[ulb]).unsqueeze(0)))
                    num_classes += 1
...
@torch.no_grad()
def label_generator_dbscan_context_kmeans(cfg, features, cuda=True, indep_thres=None, all_inds=None, **kwargs):
    assert cfg.PSEUDO_LABELS.cluster == "dbscan_context_kmeans"
