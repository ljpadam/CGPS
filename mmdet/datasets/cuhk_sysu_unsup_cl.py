...
class CUHK_SYSU_UNSUP_CLDataset(CustomDataset):
    ...
    def __init__(self,
        ...
        self.cl_pipeline = Compose(cl_pipeline)
    ...
    def prepare_train_img(self, idx):
        ...
        results_cl = deepcopy(results) 
        data = []
        data.append(self.pipeline(results))
        data.append(self.cl_pipeline(results_cl))
        return data
    
    def generate_unique_ids(self):
        # print(self.coco.anns)
        for i in range(len(self.coco.anns)):
            self.coco.anns[i]['person_id'] = i
    ...
    def load_query(self):
        ...
        for item in protocol["Query"]:
            im_name = osp.join(self.img_prefix, str(item["imname"][0, 0][0]))
            roi = item["idlocate"][0, 0][0].astype(np.int32)
            roi[2:] += roi[:2]
            probe = {}
            probe['filename'] = str(item["imname"][0, 0][0])
    ...
    def _parse_ann_info(self, img_info, ann_info):
        ...
        for i, ann in enumerate(ann_info):
            ...
            if ann.get('iscrowd', False):
                ...
                gt_labels.append([self.cat2label[ann['category_id']], ann['id'], ann['image_id']])
